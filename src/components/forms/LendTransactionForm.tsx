...
const LendTransactionForm = () => {
  ...
  const form = useForm({
    ...
    validate: {
      ...
      fromPerson: (value) =>
        tab === 'RECEIVED' && !value ? 'Received from is required' : null,
      toPerson: (value) =>
        tab === 'GIVEN' && !value ? 'Given To is required' : null,
      returnDate: (value) =>
        tab === 'GIVEN' && !value ? 'Return Date is required' : null,
    },
  });
    ...
    <div>
      ...
      >
        ...
        <div className="flex gap-x-4">
          <Select
            label={
              tab === 'RECEIVED' ? 'Credit to Account' : 'Debit from Account'
            }
            required
